...
export function EditElectionPage() {
  ...
  function fromInstant(iso: string): string {
    if (!iso) return ''
  ...
  useEffect(() => {
    ...
    electionService
      ...
      .then((el) => {
        if (el.status !== 'DRAFT') {
          setError('Solo se pueden editar elecciones en estado de borrador.')
          return
        }
        setTitle(el.title)
        setDescription(el.description || '')
        setStartDate(fromInstant(el.startDate))
        setEndDate(fromInstant(el.endDate))
        setAllowedRoles(el.allowedRoles)
        setCandidates(
          el.candidates.map((c) => ({
            name: c.name,
      ...
      .catch((err: unknown) => {
        setError(err instanceof Error ? err.message : 'Error al cargar la elección.')
        setLoading(false)
      })
  }, [id])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Error al actualizar la elección.')
      setLoading(false)
    }
  }
  ...
  if (!loaded && loading && !error) {
    return <div className="p-8 text-center text-sm text-gray-500">Cargando elección...</div>
  }
  ...
  return (
    <div className="max-w-3xl space-y-8">
      ...
      <div>
        ...
        >
          Solo se permite la edición en estado de borrador. Al guardar se
          regenerarán las claves EC.
        </p>
      </div>
      ...
      {success && (
        ...
        >
          ...
          Elección actualizada. Redirigiendo…
        </motion.div>
      )}
